...
static func create_collect_effect(position: Vector2, scene: Node):
	# Efeito de coleta de moeda melhorado
	for i in range(12):
		var particle = ColorRect.new()
		particle.size = Vector2(6, 6)
		# Variação de cores douradas
		var gold_variation = randf_range(0.7, 1.0)
		particle.color = Color(1, 0.84 * gold_variation, 0, 1)
		...
		tween.parallel().tween_property(particle, "rotation_degrees", randf_range(-180, 180), 0.6)
...
static func create_explosion_effect(position: Vector2, scene: Node, color: Color = Color(1, 0.3, 0.3, 1)):
	...
	for i in range(16):
		...
		# Variação de cor para mais impacto visual
		var color_variation = Color(
			clamp(color.r + randf_range(-0.2, 0.2), 0.0, 1.0),
			clamp(color.g + randf_range(-0.2, 0.2), 0.0, 1.0),
			clamp(color.b + randf_range(-0.2, 0.2), 0.0, 1.0),
			1.0
		)
		particle.color = color_variation
		particle.position = position
		particle.z_index = 100
		scene.add_child(particle)
		
		var angle = (i / 16.0) * TAU
		var distance = 50.0 + randf() * 30.0
		var target_pos = position + Vector2(cos(angle), sin(angle)) * distance
		
		var tween = scene.create_tween()
		tween.parallel().tween_property(particle, "position", target_pos, 0.7)
		...
		tween.parallel().tween_property(particle, "rotation_degrees", randf_range(-360, 360), 0.7)
		tween.tween_callback(particle.queue_free)
...
static func create_level_up_effect(position: Vector2, scene: Node):
	...
	for i in range(16):
		...
		var star_tween = scene.create_tween()
		star_tween.parallel().tween_property(star, "position", target_pos, 1.0)
		star_tween.parallel().tween_property(star, "modulate:a", 0.0, 1.0)
		star_tween.parallel().tween_property(star, "rotation_degrees", 360, 1.0)
		star_tween.tween_callback(star.queue_free)
	
	var label_tween = scene.create_tween()
	label_tween.parallel().tween_property(label, "position:y", label.position.y - 120, 2.0)
	label_tween.parallel().tween_property(label, "modulate:a", 0.0, 2.0)
	label_tween.parallel().tween_property(label, "scale", Vector2(1.5, 1.5), 2.0)
	label_tween.tween_callback(canvas.queue_free)
